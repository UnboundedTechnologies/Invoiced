...
import { activatePayroll, deactivatePayroll } from "@/server/actions/settings";
...
const PAYROLL_REGEX = /^\d{9}RP\d{4}$/;

export function PayrollCard({ data }: { data: SettingsRow }) {
  if (data.payrollAccountActive) {
    return <ActivePayrollPanel data={data} />;
  }
...
function ActivePayrollPanel({ data }: { data: SettingsRow }) {
  ...
  async function handleDeactivate() {
    setPending(true);
    const r = await deactivatePayroll();
    setPending(false);
    setConfirming(false);
  ...
  return (
    <Card className="border-emerald-500/30">
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <CardTitle>Payroll account: Active</CardTitle>
              <CardDescription>The salary tool is unlocked.</CardDescription>
            </div>
          </div>
      ...
      <CardContent className="space-y-4">
        <div>
          <div className="text-xs uppercase tracking-wide text-muted-foreground">RP program account</div>
          <div className="mt-1 rounded-md border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 font-mono text-sm">
            {data.payrollAccount}
          </div>
        </div>
        {!confirming ? (
          <Button variant="ghost" size="sm" className="text-rose-400 hover:bg-rose-500/10 hover:text-rose-400" onClick={() => setConfirming(true)}>
            Deactivate
          </Button>
        ) : (
          <div className="rounded-md border border-rose-500/40 bg-rose-500/5 p-3 space-y-3">
            <div className="flex gap-2 text-sm">
              ...
              <span>This will lock the salary tool again. Your past pay stubs are kept.</span>
            </div>
            <div className="flex gap-2">
              <Button size="sm" variant="destructive" disabled={pending} onClick={handleDeactivate}>
    ...
    <Card className="border-amber-500/30">
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <CardTitle>Payroll account: Not registered</CardTitle>
              <CardDescription>
                The salary tool stays locked until an RP0001 account is registered with CRA and added here.
              </CardDescription>
            </div>
          </div>
          <Switch
      ...
      {open && (
        <CardContent className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          {step === "ask" && <AskStep onYes={() => setStep("input")} onNo={() => setStep("instructions")} onCancel={() => setOpen(false)} />}
          {step === "instructions" && <InstructionsStep onBack={() => setStep("ask")} onClose={() => setOpen(false)} />}
          {step === "input" && (
            <InputStep
  ...
  return (
    <div className="space-y-3">
      <div className="text-sm font-medium">
        Have you already registered an <span className="font-mono">RP0001</span> payroll program account with CRA?
      </div>
...
function InstructionsStep({ onBack, onClose }: { onBack: () => void; onClose: () => void }) {
  const steps = [
    ...
    { title: "Add a program account", body: 'In the "Manage business" section, choose "Add a program account" → select RP – Payroll.' },
    { title: "Answer the registration questions", body: "First pay date, expected number of employees (1, yourself), pay cadence. Takes about 5 minutes." },
    { title: "Receive your RP number", body: "CRA assigns you a number that looks like 726742430RP0001. Write it down." },
    { title: "Come back here", body: 'Toggle this card on again, choose "Yes, I have the number", and paste it in.' },
  ];
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <Button asChild variant="brand" size="sm">
          <a href="https://www.canada.ca/en/revenue-agency/services/e-services/digital-services-businesses/business-account.html" target="_blank" rel="noopener noreferrer">
...
function InputStep({
  ...
  const formatOk = PAYROLL_REGEX.test(value);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        ...
        <div className="mt-1.5 relative">
          <Input
            ...
            placeholder={`${businessNumber}RP0001`}
            required
          />
          {value && (
            <span className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2">
              {allOk ? <Check className="size-4 text-emerald-400" /> : <X className="size-4 text-rose-400" />}
            </span>
          )}
        </div>
      </div>
      ...
      <ul className="space-y-1 text-xs">
        <RuleRow ok={formatOk} text="Format: 9 digits + RP + 4 digits" />
